...
ajaxloadingmsg: 'Loading Sub Menu...', //Message to show inside sub menu while fetching contents
...
ajaxload:function(tabsmenuid, submenuid, disappearBool, url){
	...
	var ajaxfriendlyurl=url.replace(regex, window.location.protocol + "//"+window.location.hostname+":" + window.location.port + "/")
...
function loadDropDownByAjax() {
  var loadByAjax = sessionStorage.getItem("loadByAjax");
  if (loadByAjax === "true") {
    // Enable New Mlp Hover over Links
    document.getElementById('mlpLinks').classList.remove("collapse");
    document.getElementById('drpDwnDwqList').classList.remove("collapse");
    document.getElementById('dwqOldLinks').classList.add("collapse");
		document.getElementById('drpDwnObcList').classList.add("collapse");
    document.getElementById('dwqIncomplete').setAttribute("href", "/indirect-dwq/client/home?Lpage_Id=ENG_INC_RES");
  } else {
    // Enable New Mlp Hover over Links
    document.getElementById('mlpLinks').classList.add("collapse");
    document.getElementById('drpDwnDwqList').classList.add("collapse");
    document.getElementById('dwqOldLinks').classList.remove("collapse");
		document.getElementById('drpDwnObcList').classList.add("collapse");
  }
}

function showDropDownAdminDiv(leftmargin)
{
	if(leftmargin == '160px')
		{
		document.getElementById('drpDwnList').style.display = 'block';
		document.getElementById('drpDwnList').style.left = '-90px';
		document.getElementById('drpDwnList').style.float = 'left';

		}
	else
		{document.getElementById('drpDwnList').style.display = 'block';
		document.getElementById('drpDwnList').style.left = '0px';
		document.getElementById('drpDwnList').style.float = 'left';
		}
}

function showDropDownOpsAdminDiv(leftmargin)
{
	if(leftmargin == '160px')
		{
		document.getElementById('drpDwnListOpsAdmin').style.display = 'block';
		document.getElementById('drpDwnListOpsAdmin').style.left = '-90px';
		document.getElementById('drpDwnListOpsAdmin').style.float = 'left';

		}
	else
		{document.getElementById('drpDwnListOpsAdmin').style.display = 'block';
		document.getElementById('drpDwnListOpsAdmin').style.left = '0px';
		document.getElementById('drpDwnListOpsAdmin').style.float = 'left';
		}
}

function hideDropDownAdminDiv()
{
	document.getElementById('drpDwnList').style.display = 'none';
}

function hideDropDownDWQDiv() {
  var loadByAjax = sessionStorage.getItem("loadByAjax");
  if (loadByAjax === "true") {
    document.getElementById('drpDwnDwqList').style.display = 'none';
  }
}

function showDropDownDWQDiv() {
  var loadByAjax = sessionStorage.getItem("loadByAjax");
  if (loadByAjax === "true") {
    document.getElementById('drpDwnDwqList').style.display = 'block';
  }
}

function hideDropDownOBCDiv() {
	var loadByAjax = sessionStorage.getItem("loadByAjax");
	if (loadByAjax === "true") {
		document.getElementById('drpDwnObcList').style.display = 'none';
	}
}

function showDropDownOBCDiv() {
	var loadByAjax = sessionStorage.getItem("loadByAjax");
	if (loadByAjax === "true") {
		document.getElementById('drpDwnObcList').style.display = 'block';
	}
}

function hideDropDownOpsAdminDiv()
{
	document.getElementById('drpDwnListOpsAdmin').style.display = 'none';
}
