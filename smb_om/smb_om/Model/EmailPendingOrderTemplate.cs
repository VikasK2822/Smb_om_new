...
{
    public class EmailPendingOrderTemplate
    {

        public static string EmailHeader(string ContactName)
        {

            string subject = "AT&T Order is Pending - Call 844-591-4852 - " + ContactName;
            return subject;
        }

        public static string EmailBody(string HEaderSalutation, string ordernumber,string SellerID, string productname, string CancelNrfcDays,bool productWithDifferentStatus, string addressbilling, string addressService)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("Hello," + HEaderSalutation);
            sb.AppendLine("<b>"+ordernumber + "</b>");
            sb.AppendLine(SellerID);
            sb.AppendLine("<b>We were not able to process your order.  Additional information is required in order to process your order.</b> ");
            sb.AppendLine("<b>IMPORTANT: In order to process your request for " + productname + ", please contact AT&T Concierge at 844-591-4852.</b>");
            sb.AppendLine("<b>Please note that your order will be automatically cancelled if we do not hear from you in  " + CancelNrfcDays + ", please contact AT&T Concierge at 844-591-4852.</b>");
            sb.AppendLine("<b>If you have ordered at least one other item, you will be receiving another email shortly regarding the balance of your order.</b>");
            ...
            sb.AppendLine("Your billing address is:");
            sb.AppendLine("Billing Address");
            sb.AppendLine(addressbilling);
            sb.AppendLine("");
            sb.AppendLine("Service Address");
            sb.AppendLine(addressService);
